...
const FooterSectionDesktop : StorefrontFunctionComponent<footerSectionLinks> = ({ ...data }) => {
    ...
    const [activeMenuIndex, updateMenuIndex] = useState(-1);

    const [isSectionActive, updateSectionStatus] = useState({
        isShop : false,
        isAbout: false,
        isHelp : false
    });

    const menuToggle = (index : number)=>{

        updateMenuIndex(-1);
        
        switch (index) {
            case 0:
                if(!isSectionActive.isShop === true) updateMenuIndex(index);
                updateSectionStatus({
                    isAbout : false,
                    isHelp : false,
                    isShop : !isSectionActive.isShop
                });
            ...
            case 1:
                if(!isSectionActive.isAbout === true) updateMenuIndex(index);
                updateSectionStatus({
                    isShop : false,
                    isHelp : false,
                    isAbout : !isSectionActive.isAbout
                });
                
            break;

            case 2:
                if(!isSectionActive.isHelp === true) updateMenuIndex(index);
                updateSectionStatus({
                    isShop : false,
                    isAbout : false,
                    isHelp : !isSectionActive.isHelp
                });
            break;       
        }
    }
    ...
    return <div className={`${styles.footer_container} fl w-100`}> 
        {
            sectionData?.length > 0 && (
                sectionData?.map((sec:sectionLinks, index:number)=> {
                    return sec?.firstlevel?.length > 0 && (
                        <div className='fl w-100'>
                            <h4 className={`${styles.toggleIconInFooter}`}>
                                {sec?.__editorItemTitle}
                                <button onClick={() => menuToggle(index)}>{index == activeMenuIndex ? '-' : '+'}</button>
                            </h4>

                            {index == activeMenuIndex && <ul className='pa0'>
                                {sec?.firstlevel?.map((link: LinkInterface) => <li key={link.text}><a href={link.url}>{link.text}</a></li>)}
                            </ul> }
                        </div >
                    )}
                )
    ...
    <div className='fl w-100 pt2'>
        <div className={`mw9 mr-auto ml-auto pv6`}>
            <div className='fl w-100'>
                    <form >
            ...
            <div className={`fl w-100`}>
                <legend className="pa0 f5 f4-ns mb3 black-80 w-100 pt7">Connect with us</legend>
